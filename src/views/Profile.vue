        ...
        <div v-if="!loading" class="row justify-center q-mt-xl q-px-md">
            <q-form class="profile-form" greedy @submit.prevent="applyChanges">
                <div class="row">
                    ...
                    <div class="col-12 col-xl-6 col-lg-6 q-my-lg q-px-md">
                        <q-input :readonly="(gymForm.id !== null)" v-model="gymForm.gym_reg_code" label="کد رجیستر"
                            lazy-rules :rules="[rules.required]"></q-input>
                    </div>
                    <div class="col-12 col-xl-12 q-px-md q-mt-lg text-center">
                        <q-btn type="submit" icon="add" rounded outline color="primary"
                            :label="gymForm.id ? 'ثبت تغییرات' : 'افزودن باشگاه'"></q-btn>
                    </div>
                </div>
            </q-form>
        </div>
